...
tf.flags.DEFINE_float("dev_percentage", 0.0005, "Percentage of the training data to use for validation")
tf.flags.DEFINE_integer('vocab_size', 50000,                             'vocab size')

# Model Hyperparameters
tf.flags.DEFINE_integer("max_sequence_length", 30, "Sequence Max Length (default: 1024)")
...
def train():
	...
	print("Loading data...")
	x, tags, deps, heads, y = data_loader.read_data(FLAGS.pos_data, FLAGS.neg_data,
	...
	def test_step(x_batch, tags_batch, deps_batch, head_batch, y_batch):
		"""
	...
	batches = data_loader.batch_iter(list(zip(x_train, tags_train, deps_train, heads_train, y_train)), FLAGS.batch_size, FLAGS.num_epochs)

	# Training loop. For each batch...
	for train_batch in batches:
		x_batch, tags_batch, deps_batch, head_batch, y_batch = zip(*train_batch)
		...
		if current_step % FLAGS.evaluate_every == 0:
			...
			index = 0
			sum_loss = 0
			test_batches = data_loader.batch_iter(list(zip(x_dev, tags_dev, deps_dev, heads_dev, y_dev)), FLAGS.batch_size, 1, shuffle=False)
			y_preds = np.ones(shape=len(y_dev), dtype=np.int)
			for test_batch in test_batches:
				x_test_batch, tags_test_batch, deps_test_batch, head_test_batch, y_test_batch = zip(*test_batch)
				preds, test_loss = test_step(x_test_batch, tags_test_batch, deps_test_batch, head_test_batch, y_test_batch)
				sum_loss += test_loss
